...
token = r'[#@_$%\w\d]{2,}'
count = CountVectorizer(token_pattern=token, max_features=1000, lowercase=False)

X_train_bag_of_words = count.fit_transform(valid_train_sentence)
X_test_bag_of_words = count.transform(valid_test_sentence)


min_value = int(0.01 * (len(training_sentence)))
#min_samples_leaf = min_value,
#,max_depth=
...
#python DT_classifier.py reviews.tsv outputDT.txt
...
plot.plot_classification_report(testing_y, predict_y, ml_name='DT',
                                  classes=labels,
                                  title='Classification report for DT_classifier') """